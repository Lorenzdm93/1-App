...
function RingCard({ viewDay }: { viewDay: string }) {
  ...
  const headline =
    sched.length > 0
      ? doneCt === sched.length
        ? t('All done 🎉')
        : t('{n} of {m} done', { n: doneCt, m: sched.length })
      : t('No scheduled habits')
  const subline =
    slips > 0
      ? (slips > 1
          ? (viewDay === todayKey() ? t('{n} slips logged today', { n: slips }) : t('{n} slips logged this day', { n: slips }))
          : (viewDay === todayKey() ? t('{n} slip logged today', { n: slips }) : t('{n} slip logged this day', { n: slips })))
      : allDone
        ? t('Great consistency today')
        : t('Tap a circle to check off')
  return (
...
function HabitCard({
  ...
  return (
    <div className={'cd-card' + (offDay || preDay ? ' muted' : '')} style={hc(habit)}>
      ...
      <button className="cd-emoji" onClick={onManage} aria-label={t('Open {name}', { name: habit.name })}>
        {habit.emoji}
      </button>
      <button className="cd-mid" onClick={onManage}>
        <span className="cd-name">
          {habit.name}
          {!isBuild && <i className="cd-badge quit">{t('quit')}</i>}
          {badge && <i className="cd-badge">{badge}</i>}
          {offDay && <i className="cd-badge">{t('rest day')}</i>}
        </span>
        ...
        <span className="cd-meta">
          {isBuild ? (
            <span className="cd-streakline">
              ...
              {habitStreak(st, habit.id) === 1 ? t('day') : t('days')}
            </span>
          ) : (
            <span className="cd-streakline">
              {t('Best clean')} <b className="num">{bestClean(st, habit.id)}</b>d
            </span>
          )}
          <span className="cd-strength">
            {t('Strength')} <i className="cd-sbar"><i style={{ width: `${str}%` }} /></i>{' '}
      ...
      <span className="cd-right">
        {isBuild ? (
          <button
            ...
            aria-pressed={done}
            aria-label={done ? t('Uncheck {name}', { name: habit.name }) : t('Check off {name}', { name: habit.name })}
          >
            <CheckGlyph />
          </button>
        ) : (
          <span className="cd-quitctl">
            <span className="cd-cleannum num">
              {slipped ? 0 : clean}
              <small> {(slipped ? 0 : clean) === 1 ? t('day clean') : t('days clean')}</small>
            </span>
            <button
              ...
              onClick={() => {
                ...
                if (!slipped && viewDay === today) {
                  toast(t('{name} — slip logged. Clean days reset; the record stays honest.', { name: habit.name }))
                }
              }}
            >
              {slipped ? t('Slipped ✓') : t('Log slip')}
            </button>
            {ms !== null && (
              <span className="cd-nextms">
                {ms - clean === 1 ? t('{n} day to {ms}', { n: ms - clean, ms }) : t('{n} days to {ms}', { n: ms - clean, ms })}
              </span>
            )}
          </span>
        )}
  ...
  return (
    <>
      <div className="cd-dayhead">
        <button className="cd-dnav" onClick={() => setViewDay((v) => shiftDay(v, -1))} aria-label={t('Previous day')}>‹</button>
        <span className="cd-dwrap">
          <i className="dow">{d.toLocaleDateString(localeTag(), { weekday: 'short', month: 'short', day: 'numeric' })}</i>
          <b className="date">{isToday ? t('Today') : d.toLocaleDateString(localeTag(), { month: 'long', day: 'numeric' })}</b>
        </span>
        <button
          className="cd-dnav"
        ...
        {!isToday && (
          <button className="cd-jump" onClick={() => setViewDay(today)}>{t('Jump to today')}</button>
        )}
      </div>
      ...
      <div className="cd-mood">
        ...
        {MOOD_FACES.map((em, i) => (
          <button
            ...
            aria-label={t('Mood {i} of 5', { i: i + 1 })}
      ...
      {habits.length === 0 ? (
        <Empty title={t('No habits yet')} sub={t("Small, daily, repeatable — that's where the 1% lives.")} />
      ) : (
      ...
      {!isToday && habits.length > 0 && (
        <p className="rs-foot">
          {t("Filling in a past day — checks carry that day's own timestamp, so streaks stay honest.")}
        </p>
      )}

      <button className="cd-addbtn" onClick={onAdd}>
        <i aria-hidden="true">＋</i> {t('Add habit')}
      </button>
    </>
  )
}
  ...
  return (
    <>
      <div className="cd-secnav">
        <button className="cd-dnav" onClick={() => move(-1)} aria-label={t('Previous month')}>‹</button>
        <b className="cd-seclabel">{new Date(ym.y, ym.m, 1).toLocaleDateString(localeTag(), { month: 'long', year: 'numeric' })}</b>
        <button className="cd-dnav" onClick={() => move(1)} disabled={isCurrent} aria-label={t('Next month')}>›</button>
      </div>

      <div className="ins-grid">
        <StatBox label={t('avg completion')} value={ins.avgCompletion !== null ? `${ins.avgCompletion}%` : '—'} />
        <StatBox label={t('active habits')} value={String(ins.activeHabits)} />
        <StatBox
        ...
        />
        <StatBox label={t('avg mood')} value={moodAvg !== null ? `${MOOD_FACES[Math.round(moodAvg) - 1]} ${moodAvg}` : '—'} />
      </div>

      {habits.length === 0 ? (
        <Empty title={t('No habits yet')} sub={t('Add some from the Today tab — the month fills in as you do.')} />
      ) : (
        <>
          <div className="card">
            <div className="cd-divider">{t('Overall — daily completion')}</div>
            <div className="cd-cal">
              {['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'].map((d) => (
                <span className="cd-caldow" key={d}>{t(d)}</span>
              ))}
          ...
          <div className="card">
            <div className="cd-divider">{t('By habit')}</div>
            {habits.map((h) => {
                ...
                <div className="cd-hrow" key={h.id} style={hc(h)}>
                  <span className="cd-hlead">
                    ...
                    <span className="cd-hmeta">
                      ...
                      <i className="cd-hsub">{!isBuild ? t('quit') : scheduleLabel(h) ?? t('daily')}</i>
                    </span>
                  </span>
                  ...
                  <span className="cd-hstats">
                    <span className="hs">
                      ...
                      <i className="l">{t('this month')}</i>
                    </span>
                    <span className="hs">
                      <b className="n num">{streak}</b>
                      <i className="l">{isBuild ? t('streak') : t('days clean')}</i>
                    </span>
                  </span>
                </div>
              )
            })}
            <p className="cd-hint">{t('Tap any cell to fill in or correct a past day. Tap a calendar day to open it.')}</p>
          </div>
        </>
      )}
    </>
...
function Heatmap({
  ...
  weeks.forEach((wk, i) => {
    ...
    if (mo !== lastMonth) {
      labels.push({ left: i * 14, text: new Date(2000, mo, 1).toLocaleDateString(localeTag(), { month: 'short' }) })
      lastMonth = mo
    }
  })
...
function YearView() {
  ...
  return (
    <>
      <div className="cd-secnav">
        <button className="cd-dnav" onClick={() => setYear((y) => y - 1)} aria-label={t('Previous year')}>‹</button>
        <b className="cd-seclabel">{year}</b>
        <button className="cd-dnav" onClick={() => setYear((y) => y + 1)} disabled={year >= nowYear} aria-label={t('Next year')}>›</button>
      </div>

      <div className="ins-grid">
        <StatBox label={t('check-ins')} value={String(yi.checkIns)} />
        <StatBox label={t('best streak')} value={`${yi.bestStreak}d`} />
        <StatBox label={t('active days')} value={String(yi.activeDays)} />
        <StatBox label={t('avg mood')} value={yi.avgMood !== null ? `${MOOD_FACES[Math.round(yi.avgMood) - 1]} ${yi.avgMood}` : '—'} />
      </div>

      {habits.length === 0 ? (
        <Empty title={t('Nothing to map yet')} sub={t('The year view fills as you do.')} />
      ) : (
        <>
          <div className="card">
            <div className="cd-divider">{t('Everything — consistency across the year')}</div>
            <Heatmap
              weeks={weeks}
          ...
          <div className="card">
              ...
              return (
                <div className="cd-yhrow" key={h.id} style={hc(h)}>
                  <div className="cd-yhhead">
                    ...
                    <b className="cd-hn">
                      {h.name}
                      {h.type === 'quit' && <i className="cd-badge quit">{t('quit')}</i>}
                    </b>
                    <span className="cd-yst">
                      <span>{t('Rate')} <b className="num">{s.rate !== null ? `${s.rate}%` : '—'}</b></span>
                      <span>{t('Best')} <b className="num">{best}</b>d</span>
                      <span>{t('Strength')} <b className="num">{strength(st, h.id)}</b></span>
                    </span>
                  </div>
                  <Heatmap
                    weeks={weeks}
            ...
            <p className="cd-hint">{t('Each square is a day — swipe sideways for the whole year. Tap a square to open that day.')}</p>
          </div>
        </>
      )}
    </>
...
function HabitForm({
  ...
  function commit() {
    if (!name.trim()) {
      toast(t('Give your habit a name first.'))
      return
    }
    if (!isQuit && schedMode === 'days' && days.length === 0) {
      toast(t('Pick at least one day.'))
      return
    }
    const schedule =
  ...
  return (
    <Sheet open title={mode === 'add' ? t('New habit') : t('Edit habit')} onClose={onClose}>
      {mode === 'add' && (
        <>
          <Field label={t('Quick start — tap to prefill')}>
            <div className="cd-qk">{t('Build')}</div>
            ...
            <div className="cd-qk">{t('Quit')}</div>
            <div className="cd-qchips">
          ...
          <div className="cd-divider">{t('or set up your own')}</div>
        </>
      )}
      ...
      <Field label={t('Type')}>
        <div className="cd-typeseg">
          <button className={'opt' + (!isQuit ? ' sel' : '')} onClick={() => setType('build')}>
            <b>{t('🎯 Build')}</b>
            <i>{t('Do it — check off each day')}</i>
          </button>
          <button className={'opt q' + (isQuit ? ' sel' : '')} onClick={() => setType('quit')}>
            <b>{t('🚫 Quit')}</b>
            <i>{t('Avoid it — count days clean')}</i>
          </button>
        </div>
      </Field>
      ...
      <Field label={t('Name')}>
        <input
          className="tinput"
          placeholder={isQuit ? t('e.g. No smoking') : t('e.g. Morning workout')}
          value={name}
          maxLength={60}
      ...
      {!isQuit && (
        <Field label={t('Repeat')}>
          <div className="cd-minitabs">
            <button className={schedMode === 'daily' ? 'on' : ''} onClick={() => setSchedMode('daily')}>{t('Every day')}</button>
            <button className={schedMode === 'days' ? 'on' : ''} onClick={() => setSchedMode('days')}>{t('Specific days')}</button>
          </div>
          {schedMode === 'days' && (
      ...
      <Field label={isQuit ? t('Quit date') : t('Start date')}>
        <input
          className="tinput"
          type="date"
          value={startDate}
        ...
        />
        <p className="cd-hint">
          {isQuit ? t('The day you quit — your clean streak counts from here.') : t("Days before this aren't counted.")}
        </p>
      </Field>

      <Field label={t('Cue / reminder (optional)')}>
        <input
          className="tinput"
          placeholder={isQuit ? t('e.g. When I crave it, I drink water instead') : t('e.g. After I make coffee, I meditate for 10 min')}
          value={cue}
          maxLength={140}
          onChange={(e) => setCue(e.target.value)}
        />
        <p className="cd-hint">{t('A specific “when/where” cue makes habits far more likely to stick.')}</p>
      </Field>
      ...
      {!isQuit && (
        <Field label="Daily time goal — links Grove / Respiro / Ora minutes to this habit (0 = off)">
          <div className="cd-target">
            ...
            <b className="num">{targetMin === 0 ? t('off') : targetMin >= 60 ? `${Math.floor(targetMin / 60)}h ${String(targetMin % 60).padStart(2, '0')}` : `${targetMin}m`}</b>
      ...
      <button className="btn btn-primary cd-commit" onClick={commit}>
        {mode === 'add' ? t('Add habit') : t('Save changes')}
      </button>
    </Sheet>
  )
}
...
function ManageSheet({ habit, onClose }: { habit: Habit | null; onClose: () => void }) {
  ...
  return (
    <>
      {!editing && (
        <Sheet open title={`${live.emoji} ${live.name}`} onClose={onClose}>
          <div style={hc(live)}>
            ...
            <p className="cd-hint">{t('Last 8 weeks — today outlined.')}</p>
            <div className="kv">
              <span className="k">{isBuild ? t('Current streak') : t('Days clean')}</span>
              <span className="num">{isBuild ? habitStreak(st, live.id) : daysClean(st, live.id)} {t('days')}</span>
            </div>
            <div className="kv">
              <span className="k">{isBuild ? t('Best streak') : t('Best clean run')}</span>
              <span className="num">{isBuild ? bestBuildStreak(st, live.id) : bestClean(st, live.id)} {t('days')}</span>
            </div>
            <div className="kv">
              <span className="k">{t('Strength')}</span>
              <span className="num">{strength(st, live.id)} / 100</span>
            </div>
            <div className="kv">
              <span className="k">{t('Schedule')}</span>
              <span>{isBuild ? scheduleLabel(live) ?? t('Every day') : t('Every day')}</span>
            </div>
            {live.cue && (
              <div className="kv">
                <span className="k">{t('Cue')}</span>
                <span>{live.cue}</span>
              </div>
            )}
            <div className="kv">
              <span className="k">{isBuild ? t('Started') : t('Quit date')}</span>
              <span>
                {new Date(live.startDate + 'T12:00:00').toLocaleDateString(localeTag(), {
                  day: 'numeric', month: 'short', year: 'numeric',
                })}
              </span>
            </div>
            <div className="btn-row" style={{ marginTop: 14 }}>
              <button className="btn btn-danger" onClick={() => setConfirmArchive(true)}>{t('Archive')}</button>
      ...
      <ConfirmSheet
        open={confirmArchive}
        title={t('Archive this habit?')}
        body={t('It disappears from your list but its history stays in your data.')}
        actionLabel={t('Archive')}
        danger
        onConfirm={() => {
          archiveHabit(habit.id)
          onClose()
          toast(t('Habit archived'))
        }}
